...
def _register_fonts() -> None:
    ...
    try:
        ...
        log.warning("finance.export.fonts.skip", extra={"_extra_fields": {"path": str(fonts_dir)}})
        return
    FONT = "App"
    FONT_BOLD = "App-Bold"
...
def _logo_flowable(tenant: Tenant) -> Image | None:
    ...
    try:
        ...
        log.warning("finance.export.logo.skip", extra={"_extra_fields": {"path": str(path)}})
        return None
    if ratio >= 1:
        img.drawWidth = 34 * mm
...
async def export_finance_statement(
    db: AsyncSession,
    tenant_id: uuid.UUID,
    statement: str,
    fmt: str,
    ...
    if statement == "balance-sheet":
        report = await finance_service.balance_sheet(db, tenant_id, as_of or datetime.now(UTC).date())
    elif statement == "general-ledger":
        report = await finance_service.general_ledger(db, tenant_id, from_date, to_date, account_id)
    elif statement == "trial-balance":
        report = await finance_service.trial_balance(db, tenant_id, from_date, to_date)
    elif statement == "income-statement":
        report = await finance_service.income_statement(db, tenant_id, from_date, to_date)
    else:  # cash-flow
        report = await finance_service.cash_flow(db, tenant_id, from_date, to_date)
