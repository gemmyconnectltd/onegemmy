...
class Budget(UUIDPKMixin, TenantScopedMixin, TimestampMixin, Base):
    ...
    account_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("finance_accounts.id", ondelete="CASCADE"), nullable=False
    )
    ...
    __table_args__ = (
        Index("uq_finance_budgets_tenant_account_period", "tenant_id", "account_id", "period", unique=True),
        Index("ix_finance_budgets_tenant_id", "tenant_id"),
        Index("ix_finance_budgets_account_id", "account_id"),
    )
