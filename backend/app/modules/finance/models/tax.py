...
class TaxConfig(UUIDPKMixin, TenantScopedMixin, TimestampMixin, Base):
    ...
    __table_args__ = (
        Index("uq_finance_tax_configs_tenant_type", "tenant_id", "tax_type", unique=True),
        Index("ix_finance_tax_configs_tenant_id", "tenant_id"),
        Index("ix_finance_tax_configs_tax_type", "tax_type"),
    )


class TaxCalculation(UUIDPKMixin, TenantScopedMixin, TimestampMixin, Base):
    """Individual tax calculation records"""
    __tablename__ = "finance_tax_calculations"
    ...
    __table_args__ = (
        Index("ix_finance_tax_calculations_tenant_id", "tenant_id"),
        Index("ix_finance_tax_calculations_period", "period"),
        Index("ix_finance_tax_calculations_type_status", "calculation_type", "status"),
    )


class TaxPayment(UUIDPKMixin, TenantScopedMixin, TimestampMixin, Base):
    """Tax payment records for tracking payments to RRA"""
    __tablename__ = "finance_tax_payments"
    ...
    __table_args__ = (
        Index("uq_finance_tax_payments_tenant_ref", "tenant_id", "payment_reference", unique=True),
        Index("ix_finance_tax_payments_tenant_id", "tenant_id"),
        Index("ix_finance_tax_payments_period", "period"),
    )
