...
export interface RestockValues {
  ...
  reason: string;
  notes: string;
  newStock?: number; // only for adjust
}
...
export function RestockDrawer({ open, onClose, productName, currentStock, onSubmit }: RestockDrawerProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!valid) return;
    onSubmit({ mode, qty: qtyNum, reason, notes, newStock });
    reset();
    onClose();
  };
    ...
    <Drawer
      ...
      footer={
        <form onSubmit={handleSubmit}>
          <FormFooter
            submitLabel={mode === "restock" ? "Add Stock" : "Apply Adjustment"}
