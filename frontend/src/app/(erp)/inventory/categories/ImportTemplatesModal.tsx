...
import { X, Search, FolderClosed, CheckCheck, Sparkles } from "lucide-react";
import { Button } from "@/components/ui/Button";
import { useCategoryTemplates, useImportCategories } from "@/lib/api/hooks";
import type { ApiCategoryTemplate } from "@/lib/api";
...
export default function ImportTemplatesModal({ open, onClose, color }: Props) {
  ...
  const { data, isLoading } = useCategoryTemplates({ enabled: open });
  ...
  const active: ApiCategoryTemplate | undefined = templates.find((t) => t.id === activeId);
  ...
  async function handleImport() {
    if (allSelectedNames.length === 0 || importCategories.isPending) return;
    try {
      await importCategories.mutateAsync(allSelectedNames);
      close();
    } catch { /* ignore */ }
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div role="dialog" aria-modal="true" className="relative bg-card border border-border shadow-2xl w-full max-w-6xl h-[85vh] rounded-2xl flex flex-col overflow-hidden animate-drawer-center">
        <div className="flex items-center justify-between px-6 py-4 border-b border-border flex-shrink-0">
          <h2 className="text-lg font-bold text-foreground">Import Category Templates</h2>
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-[11px] text-muted mt-1 flex items-center gap-1.5">
                        <FolderClosed size={11} /> {count} Categories
                          ...
                          {g.items.map((item) => {
                            ...
                            return (
                              <button
                                ...
                                title={alreadyExists ? "Already in your categories" : undefined}
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-border flex-shrink-0">
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-lg bg-surface flex items-center justify-center">
              <FolderClosed size={15} className="text-muted" />
            </div>
            <div>
              <p className="text-[13px] font-bold text-foreground">{allSelectedNames.length} Items Selected</p>
              <p className="text-[11px] text-muted">Across {groupsWithSelections} category group{groupsWithSelections === 1 ? "" : "s"}</p>
            </div>
          </div>
          <div className="flex gap-2">
            <Button type="button" variant="secondary" onClick={close} className="rounded-lg">Cancel</Button>
            <Button onClick={handleImport} disabled={allSelectedNames.length === 0 || importCategories.isPending} className="rounded-lg">
              {importCategories.isPending ? "Importing…" : "Import Categories"}
            </Button>
          </div>
        </div>
      </div>
